...
export class TreoMediaWatcherService {
  private _onMediaChange: BehaviorSubject<{
  ...
  private _init(): void {
    ...
    this._breakpointObserver
      .observe(Object.values(treoBreakpoints))
      .subscribe(state => {
        ...
        for (const [query, matches] of Object.entries(state.breakpoints)) {
          ...
          const alias = Object.keys(treoBreakpoints).find(
            key => (treoBreakpoints as any)[key] === query
          );

          // Prepare the observable values
          matchingAliases.push(alias);
          (matchingRules as any)[alias] = query;
        }
